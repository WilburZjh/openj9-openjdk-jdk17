...
public final class RestrictedSecurity {
    ...
    static {
        supportedPlatformsNSS.put("Arch", List.of("amd64"));
     ...
     * the restrictedsecurity.profile system property is used to set a
     * specific security profile or one of the system properties semreu.fips
     * or semeru.fips140-3 is set to true.
     *
    ...
    private static void getProfileID(Properties props) {
        ...
        if (selectedProfile.indexOf(".") != -1) {
                        ...
                        if (key.endsWith(".desc.default")) {
                            ...
                            if (Boolean.parseBoolean(props.getProperty(key))) {
                                ...
                                if (defaultMatch == null) {
                                    ...
                                    printStackTraceAndExit("Multiple default profiles for " + selectedProfile);
                                }
                            }
                        }
                    }
            ...
            if (defaultMatch == null) {
                printStackTraceAndExit("No default profile was found for " + selectedProfile);
            } else {
    ...
    private static void checkIfKnownProfileSupported() {
        if (profileID.contains("NSS") && !isNSSSupported) {
            printStackTraceAndExit("NSS profiles are not supported in this platform.");
        }

        if (profileID.contains("OpenJCEPlus") && !isOpenJCEPlusSupported) {
            printStackTraceAndExit("OpenJCEPlus profiles are not supported in this platform.");
        }

        if (debug != null) {
            debug.println("Profile is supported in this platform.");
        }
    }
    ...
    private static void checkFIPSCompatibility(Properties props) {
        ...
        if (isFIPSProfile) {
            if (debug != null) {
                debug.println("Profile specified as FIPS compliant.");
            }
            isFIPSEnabled = true;
        } else {
            printStackTraceAndExit(profileID + " profile is not specified as FIPS compliant.");
        }
    }
    ...
    private static final class RestrictedSecurityProperties {
        ...
        private void initProviders() {
            ...
            if (providers.isEmpty()) {
                printStackTraceAndExit(
                        "No providers were provided as part of the Restricted Security profile.");
            }
        ...
        private void listAvailableProfiles() {
            ...
            System.out.println("Those are the available Restricted Security profiles:\n");
